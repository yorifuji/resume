# resume

依藤 充範（YORIFUJI MITSUNORI）

# 業務経歴

- 自社開発のWeb会議サービスの設計・開発
- 通信キャリアのR&D受託案件の設計・開発
- AWSを使ったクラウドサービスの設計・開発
- プロジェクトマネジメント業務
- エンジニアチームのマネジメント
- 新規事業開発のPoCなど

# スキル

- 各種クライアントOSで利用するネイティブアプリケーションの開発
    - Win / macOS / iOS / Android
- 複数のOSがネットワーク上に混在する状況でシームレスに機能するアプリケーションの設計・開発
- 遠隔協調作業を想定したアプリケーションの設計・開発
  - 画面共有・ドキュメント操作・ホワイトボードなど
- TCP/UDPを使った通信処理の実装
    - IPv6対応
- 各種プログラミング言語を利用したアプリケーション開発
    - C/C++/Objective-c/Java
- 各種フレームワーク・ライブラリを活用したアプリケーション開発
    - Win32API/AppKit/UIKit
- OSSの映像コーデックのモバイルアプリケーションへの組み込み
- AWSでマネージドサービスを利用したクラウドサービスの開発
- オンプレミス・イントラでの利用を想定したパッケージアプリケーションの開発
    - RHEL(Red Hat Enterprise Linux)、CentOS等
    - OS・ミドルウェアに関する知識・経験
- 後方互換性を考慮したアプリケーションのアーキテクチャ設計・開発
- 使用可能な開発ツール
    - Git/SVN/Docker/シェルスクリプト

# ソフトスキル

- 事業の意思決定の経験
  - プロダクトの投資計画の策定
- プロジェクトマネジメント業務
  - スケジュール管理、品質管理、リスク管理
- プロダクト開発における主導的な役割
  - ビジネスサイドや他部署との利害調整、コミュニケーション
  - 顧客要望を理解した上で実現可能な仕様に落とし込むプロセス
  - プロダクトのロードマップの作成や継続的な改善作業
- エンジニアチームのマネジメントの経験
- ユーザサポートやトラブルシューティングの経験
  - 営業活動支援、システムの設置作業
- オフショア開発の管理業務

# 意欲・興味

- 少人数チームでのアジャイル開発で小さく早くリリースをして仮説検証を行うプロセスに興味があります
- モバイルアプリが好きですがバックエンドやフロントエンドにも興味があります
  - AWS や Firebase などの mBaaS、Reactなど
- 発信活動や新しい技術の習得に対して興味があります

# 保有資格

- 応用情報技術者 （2017/12）
- [AWS クラウドプラクティショナー](https://www.credly.com/badges/ad001cb9-c302-4547-a26e-49d97d61cd3f/public_url) （2021/8）

# 最終学歴

- 東京農工大学大学院 工学教育部 情報コミュニケーション工学専攻 博士前期課程 修了（2005/3）

# SNS、ブログなど

- GitHub [https://github.com/yorifuji](https://github.com/yorifuji)
- Zenn [https://zenn.dev/yorifuji](https://zenn.dev/yorifuji)
- Twitter [https://twitter.com/yorifuji](https://twitter.com/yorifuji)
- Qiita [https://qiita.com/yorifuji](https://qiita.com/yorifuji)
- Note [https://note.com/yorifuji](https://note.com/yorifuji)

# 登壇資料

- iOSのCallKitフレームークとSkyWayについて (2019/08/21)
  - https://speakerdeck.com/yorifuji/callkit-skyway

# 個人開発

- ビデオ通話アプリ
  - WebRTCを使ったブラウザベースのビデオ通話アプリ
  - 技術スタック
    - Vue.js
    - Bootstrap
    - WebRTC
  - デモサイト
    - https://github.com/yorifuji/meets
- [Sweep VoIP 通話アプリ](https://apps.apple.com/jp/app/sweep-voip-%E9%80%9A%E8%A9%B1%E3%82%A2%E3%83%97%E3%83%AA/id1464355224)
  - iOSでVoIPを使った音声通話アプリ
  - 技術スタック
    - UIKit
    - PushKit
    - CallKit
    - WebRTC
    - AWS
- [よみとり OCR](https://apps.apple.com/jp/app/%E3%82%88%E3%81%BF%E3%81%A8%E3%82%8A/id1541364743)
  - 画像から文字認識をするOCRアプリ
  - 技術スタック
    - SwiftUI
    - Firebase Machine Learning
- 音声SNSアプリ
  - 多人数で雑談するソーシャルアプリ
  - 技術スタック
    - SwiftUI
    - Firebase
    - WebRTC
